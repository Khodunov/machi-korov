...
.venv/bin/python skills/generate-card/scripts/remove_light_background.py \
  buildings/upravlyayushchaya-kompaniya-v1-source.png \
  buildings/upravlyayushchaya-kompaniya-v1.png

.venv/bin/python skills/generate-card/scripts/card_compositor.py \
  --template backgrounds/green.png \
  --overlay buildings/upravlyayushchaya-kompaniya-v1.png \
  --title-icon icons/house.png \
  --shadow \
  --output cards/upravlyayushchaya-kompaniya-v1.png \
  --x-frac 0.50 \
  --y-frac 0.50 \
  --scale 0.67 \
  --coin-number 2 \
